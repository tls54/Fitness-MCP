...
TOKEN_FILE = os.environ.get(
    "STRAVA_TOKEN_PATH", os.path.join(os.path.dirname(os.path.abspath(__file__)), "strava_token.json")
)
CONFIG_FILE = os.environ.get(
    "STRAVA_CONFIG_PATH", os.path.join(os.path.dirname(os.path.abspath(__file__)), "strava_user_config.json")
)
...
def _reverse_geocode(latlng: list) -> str | None:
    ...
    try:
        response = requests.get(
            ...
            headers={"User-Agent": "fitness-mcp/1.0"},
            timeout=5,
        )
        data = response.json()
...
def register(mcp: FastMCP) -> None:
    ...
    def get_activity_streams(activity_id: int, stream_types: str = "heartrate,velocity_smooth,cadence,altitude") -> str:
        ...
        For summary insights (zone distribution, HR drift, effort quality) use strava_get_activity_analysis instead.
